...
async def scroll_for_lazyload(page: Page) -> None:
    ...
    쿠키 동의 버튼 및 상담사 팝업 클릭으로 인한 페이지 리다이렉트가 발생해도 안전하게 스크롤을 계속합니다.
    """
    ...
    try:
        ...
        while True:
            try:
                ...
                # 상담사 팝업(LineClose 버튼) 처리 - 스크롤 중 매번 체크
                try:
                    consultant_result = await page.evaluate("""
                        () => {
                            const result = {
                                minimizedFound: 0,
                                minimizedClosed: 0,
                                ...
                                expandedClosed: 0,
                                secondCheckClosed: 0,
                                chatTriggerBoxFound: false,
                                allLineCloseCount: 0
                            };
                            
                            // 추가 디버깅: 상담사 관련 요소 전체 탐색
                            const chatTriggerBox = document.querySelector('.spr-chat__trigger-box');
                            result.chatTriggerBoxFound = !!chatTriggerBox;
                            
                            const allLineClose = document.querySelectorAll('svg[data-icon-name="LineClose"]');
                            result.allLineCloseCount = allLineClose.length;
                            
                            // 우선순위 1: 최소화 상태 (얼굴만) - 전체 상담사 위젯 제거
                            const triggerContainers = document.querySelectorAll('[data-testid="triggerContainer"], .spr-chat__trigger-box');
                            result.minimizedFound = triggerContainers.length;
                            
                            triggerContainers.forEach(container => {
                                try {
                                    // proactive-box가 없으면 최소화 상태
                                    const hasProactiveBox = document.querySelector('.spr-chat__proactive-box');
                                    if (!hasProactiveBox || window.getComputedStyle(hasProactiveBox).display === 'none') {
                                        const rect = container.getBoundingClientRect();
                                        const isVisible = rect.width > 0 && rect.height > 0 && 
                                                         window.getComputedStyle(container).visibility !== 'hidden' &&
                                                         window.getComputedStyle(container).display !== 'none';
                                        if (isVisible) {
                                            // 전체 위젯의 최상위 컨테이너 찾아서 제거
                                            let rootContainer = container.closest('.css-fc9dy6, [class*="ezg1tqb"]');
                                            if (!rootContainer) {
                                                rootContainer = container.parentElement;
                                                while (rootContainer && !rootContainer.classList.contains('css-fc9dy6')) {
                                                    rootContainer = rootContainer.parentElement;
                                                    if (rootContainer === document.body) break;
                                                }
                                            }
                                            
                                            if (rootContainer && rootContainer !== document.body) {
                                                rootContainer.remove();
                                                result.minimizedClosed++;
                                            }
                                        }
                                    }
                                } catch(e) {
                                    console.error('Error closing minimized popup:', e);
                                }
                            });
                            
                            // 우선순위 2: 확장 상태 (얼굴+글) - iframe 포함 전체 위젯 제거
                            
                            // 2-1: proactive-box (글 부분 iframe)
                            const proactiveBoxes = document.querySelectorAll('.spr-chat__proactive-box, div[class*="spr-chat__proactive"]');
                            proactiveBoxes.forEach(proactiveBox => {
                                try {
                                    const rect = proactiveBox.getBoundingClientRect();
                                    const style = window.getComputedStyle(proactiveBox);
                                    const isVisible = rect.width > 0 && rect.height > 0 && 
                                    ...
                                    if (isVisible) {
                                        result.expandedFound++;
                                        
                                        // iframe과 부모를 모두 숨김
                                        proactiveBox.remove();
                                        
                                        result.expandedClosed++;
                                    }
                                } catch(e) {
                                    console.error('Error closing expanded popup:', e);
                                }
                            });
                            
                            // 2-2: iframe을 직접 찾아서 제거
                            const chatIframes = document.querySelectorAll('iframe[name*="spr-chat"], iframe[aria-label*="chat" i], iframe[title*="chat" i]');
                            chatIframes.forEach(iframe => {
                                try {
                                    // proactive frame (글 부분)만 제거 (trigger frame은 유지)
                                    if (iframe.name && iframe.name.includes('proactive')) {
                                        const rect = iframe.getBoundingClientRect();
                                        if (rect.width > 0 && rect.height > 0) {
                                            // iframe의 최상위 부모 컨테이너 찾기
                                            let container = iframe.closest('.spr-chat__proactive-box');
                                            if (!container) {
                            ...
                            // 추가: LineClose SVG도 계속 시도 (다른 팝업 대비)
                            const allLineCloseSvgs = document.querySelectorAll('svg[data-icon-name="LineClose"]');
                            allLineCloseSvgs.forEach(svg => {
                                try {
                                    const inTriggerClose = svg.closest('[data-testid="triggerCloseIconContainer"]');
                                    if (!inTriggerClose) {
                                    ...
                                    console.error('Error closing LineClose button:', e);
                                }
                            });
                            
                            // 확장 상태를 닫은 후 300ms 뒤 최소화 상태 재확인
                            if (result.expandedClosed > 0) {
                                setTimeout(() => {
                                    // 남아있는 상담사 위젯 완전 제거
                                    const remainingChatWidgets = document.querySelectorAll('.css-fc9dy6, [class*="ezg1tqb"], .spr-chat__trigger-box');
                                    remainingChatWidgets.forEach(widget => {
                                        try {
                                            const rect = widget.getBoundingClientRect();
                                            if (rect.width > 0 && rect.height > 0) {
                                                widget.remove();
                                                result.secondCheckClosed++;
                                            }
                                        } catch(e) {
                                            console.error('Error in second check:', e);
                                        }
                                    });
                                }, 300);
                            }
                    ...
                    if consultant_result:
                        # 디버깅: 상담사 요소 감지 여부
                        chat_trigger_found = consultant_result.get('chatTriggerBoxFound', False)
                        all_line_close = consultant_result.get('allLineCloseCount', 0)
                        
                        if chat_trigger_found or all_line_close > 0:
                            log.info(f"Consultant popup detected - ChatTriggerBox: {chat_trigger_found}, Total LineClose SVG: {all_line_close}")
                        
                        if consultant_result.get('minimizedFound', 0) > 0:
                            log.info(f"Consultant popup - Minimized state: found={consultant_result['minimizedFound']}, closed={consultant_result['minimizedClosed']}")
                        if consultant_result.get('expandedFound', 0) > 0:
                            log.info(f"Consultant popup - Expanded state: found={consultant_result['expandedFound']}, closed={consultant_result['expandedClosed']}")
                        
                        total_closed = consultant_result.get('minimizedClosed', 0) + consultant_result.get('expandedClosed', 0)
                        if total_closed > 0:
                            await asyncio.sleep(0.8)  # 확장→최소화→닫기 시간 고려
                            if consultant_result.get('secondCheckClosed', 0) > 0:
                                log.info(f"Consultant popup - Second check closed: {consultant_result['secondCheckClosed']}")
                                
                except Exception as e:
                    log.warning(f"Error handling consultant popup during scroll: {e}")
